...
// Force CUDA init
    ...
    if (backend != nullptr) {
        ...
        fprintf(stderr, "[Inker Engine] CUDA init failed, falling back.\n");
    }
#endif
    ...
    if (backend == nullptr) {
        ...
        if (backend != nullptr) {
            fprintf(stderr, "[Inker Engine] CPU fallback used.\n");
        }
    }

    if (backend == nullptr) {
        if (error_msg != nullptr) {
            const char * name = component_name != nullptr ? component_name : "component";
